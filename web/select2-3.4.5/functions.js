function formatResult(result) {
    ...
    if (result.type && result.type != '' ) {
        ...
        if( result.type == "club" ) {
            returnStr += "<span class='icon-group' style='color:#F8C70B'></span> Demande d'adhésion au club : ";
        }
...
function initSelect2ForResearch( select, translations ) {
    select.select2({
        ...
        if( selectElement.type == "club" ) {
            ...
            window.location.href = Routing.generate('ksClub_askForMembership_V2', {'clubId': selectElement.id});
        }
        if( selectElement.type == "article" ) {
            window.location.href = Routing.generate('ksWikisport_show', {'id' : selectElement.id});
        }
        if( selectElement.type == "video" ) {
            window.location.href = Routing.generate('ksActivity_showActivity', {'activityId' : selectElement.id});
        }
