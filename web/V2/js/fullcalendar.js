	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			...
			if (url) {
				...
				pushLoading();
				if (ajax) ajax.abort(); //CED
				ajax = $.ajax($.extend({}, ajaxDefaults, source, { //CED
					data: data,
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		if (opt('allDaySlot')) {
			...
			s =
				...
				"<tr>" +
				//FMO : "<th style='' class='" + headerClass + " fc-agenda-axis'>" + opt('allDayText') + "</th>" +
                                //"<th style='' class='" + headerClass + " fc-agenda-axis'>" + opt('allDayText') + "</th>" +
				"<td>" +
		...
		for (i=0; d < maxd; i++) {
			...
			s +=
				//FMO : display:none
                                "<tr style='display:none' class='fc-slot" + i + ' ' + (!minutes ? '' : 'fc-minor') + "'>" +
	...
	function buildDayTableHeadHTML() {
		...
		if (showWeekNumbers) {
			...
			html += "<th id='gutter1' class='fc-agenda-axis " + headerClass + "'>&nbsp;</th>";
		}
	...
	function buildDayTableBodyHTML() {
		...
		html +=
			"<tbody>" +
			"<tr>" +
			"<th id='gutter2' class='fc-agenda-axis " + headerClass + "'>&nbsp;</th>";
...
function AgendaEventRenderer() {
	...
	function slotSegHtml(event, seg) {
		...
		html +=
			...
			"</div>" +
			//FMO : ajout <center> et mise en commentaire de htmlEscape(event.title)
                        "<center><div class='fc-event-title'>" +
...
function DayEventRenderer() {
	...
	function buildHTMLForSegment(segment) {
		...
		html +=
                        //FMO : ajout <center> et mise en commentaire de htmlEscape(event.title)
			"<center><span class='fc-event-title'>" +
			//htmlEscape(event.title || '') +
			"</span></center>" +
			"</div>";
	...
	function setVerticals(segments, doRowHeights) {
		...
		for (var i=0; i<rowContentElements.length; i++) {
			rowContentTops.push(
				rowContentElements[i].offset().top -420 //FMO : fonctionne uniquement avec l'agenda année pour la page competitions
			);
                }
		...
		segmentElementEach(segments, function(segment, element) {
		        element.css(
				'top',
				rowContentTops[segment.row] + segment.top //FMO +25 pour ne pas cacher le numéro du jour
			);
		});
	}
