...
(function($) {
	...
	$.fn.scrollbar = function(params) {
		...
		params = $.extend( {
			taille_englobe: "auto",//screen.height - 750,			//Taille de l'espace visible - /!\ Doit �tre un nombre ou 'auto'
			...
			pas:30,							//Pas du scroll molette - /!\ Doit �tre un nombre ou 'auto'
		...
		return this.each(function() {
			...
			function calcul_contenu_top_max(){//console.log('params.taille_englobe:'+params.taille_englobe+" taille_contenu:"+taille_contenu)
			...
			function deplacement_bouton(info_top_contenu){
                ...
                //alert(depl_bouton);//alert(calcul_bouton_top_max());
			...
			function calcul_hauteur_auto(){
				if(params.taille_englobe == "auto"){
					//padTop = $('#content_body').css('padding-top');
					//padTop = padTop.substring(0,padTop.length-2);
					
					//padBot = $('#content_body').css('padding-bottom');
                    ...
                    //padBot = padBot.substring(0,padBot.length-2);
					params.taille_englobe = padBot - padTop;
			...
			if(controle_donnee()){
				...
				//$$.wrapInner2('<div id="englobe"></div>');
                //var content = $$.html();
                //$$.empty().html('<div id="englobe">' + content + '</div>');

                 var div = document.createElement("div");
                div.id = "englobe";

                var content_body = document.getElementById('content_body');
                var content_body_childnodes = content_body.childNodes;
                var nbFils = content_body_childnodes.length;
                // Move the body's children into bodywrap
                for(var i = 0; i < nbFils; i++){
                   // document.write(fils[i].nodeType +"/"+ fils[i].nodeValue +"<br>");
                   //alert(content_body_childnodes[i].nodeType +"/"+ content_body_childnodes[i].nodeValue +"<br>")
                    if(content_body_childnodes[i]) {
                        if(content_body_childnodes[i].nodeType==1){//alert(content_body_childnodes[i].nodeType +"/"+ content_body_childnodes[i].childNodes[0].nodeValue)
                            //document.write(fils[i].nodeType +"/"+ fils[i].childNodes[0].nodeValue +"<br>");
                            div.appendChild(content_body_childnodes[i]);
                        }
                    }
                }
                //var content = content_body.innerHTML;
                //div.innerHTML = content;
                /*while (content_body.childNodes)
                {
                    div.appendChild(content_body.firstChild);
                }*/

                // Append bodywrap to the body
                content_body.appendChild(div);
                
                /*var superdiv = document.createElement("div");
                superdiv.id = "superwrap";

                // Move the body's children into superwrap
                while (document.body.firstChild)
                {
                    superdiv.appendChild(document.body.firstChild);
                }

                // Append superwrap to the body
				...
				if(params.molette){
					$$.mousewheel(function(event, delta) {
						...
						//Si le delta est n�gatif, c'est � dire que l'on "ram�ne" la molette
							...
							//if(top_bouton > calcul_bouton_top_max()){top_contenu = calcul_contenu_top_max()}
                            //console.log("topcontenu :" + top_contenu +" top_max : " +calcul_contenu_top_max());
